function [C,phi,S12,S21,S1,S2,f,confC,phistd,Cerr]=coherencyc(data1,data2,params)
% added output S21 KB
%
...
%end;
N=check_consistency(data1,data2); % checks data consistency, returns data length
nfft=max(2^(nextpow2(N)+pad),N); % finds next 2^n based on data length, optimizes fft
[f,findx]=getfgrid(Fs,nfft,fpass); % gets the frequency grid associated with a given fft
tapers=dpsschk(tapers,N,Fs); % check tapers
J1=mtfftc(data1,tapers,nfft,Fs); % performs fft
J2=mtfftc(data2,tapers,nfft,Fs);
J1=J1(findx,:,:); J2=J2(findx,:,:);
%S12=squeeze(mean(conj(J1).*J2,2)); % edited
%S21=squeeze(mean(conj(J2).*J1,2)); % added, then edited
S12=squeeze(mean(J1.*conj(J2),2)); %kb nov12 2019
S21=squeeze(mean(J2.*conj(J1),2)); %kb nov12 2019
S1 =squeeze(mean(conj(J1).*J1,2));
S2 =squeeze(mean(conj(J2).*J2,2));
% added 'S21=squeeze(mean(S21,2));' in next line of code   KB
if trialave; S12=squeeze(mean(S12,2));S21=squeeze(mean(S21,2)); S1=squeeze(mean(S1,2)); S2=squeeze(mean(S2,2)); end;
C12=S12./sqrt(S1.*S2);
C=abs(C12); 
phi=angle(C12);
if nargout>=9; 
